...
#####################################################################
plotSpeciesMonths <- function(species="Rufous Hummingbird",
                              parks=levels(factor(sp.months$Site)),
                              colors=sample(c("forestgreen","cornflowerblue","gold","orangered","violet"))){
  
sub <- subset(sp.months,Species %in% species
              & Site %in% parks)

#this whole chunk just puts in rows for 0 months
temp <- sub[0,]
plot.data <- sub[0,]
for(j in levels(factor(sub$Site))){ #loop over sites
    site <- subset(sub,Site==j)
    for(k in levels(factor(sub$Species))){ #loop over species
